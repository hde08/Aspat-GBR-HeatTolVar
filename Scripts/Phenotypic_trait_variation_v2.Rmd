---
title: "Phenotypic variation analyses GCB paper"
author: "Hugo DENIS"
date: "2022-12-26"
...
source(paste0(path,"Scripts/heat_tol_variation_plotting_functions.R"))
```

#Read data 
```{r}
#Phenotypic data
pheno_data=read.csv(paste0(path,"Results/Phenotypic_trait_per_genotype.csv"),header=T)
...
write.csv(GH,"D:/Hugo/Results/1st_paper/GH_posthoc_phenotype.csv",row.names=F)
```
...
adj_density_plot1=ed50_adj %>% ggplot(aes(ed50.fvfm_adj)) +
  ...
  annotate("text", x = c(35), 
           y = c(15), 
           adj = 0, label = paste0("5%-95%\nrange = ", round(quantile(res.fvfm$ed50.fvfm_adj,probs=c(.05,.5, .95))[3]-quantile(res.fvfm$ed50.fvfm_adj,probs=c(.05,.5, .95))[1], 2)), size = 5)
...
adj_density_plot2=ed50_adj %>% ggplot(aes(ed50.ndvi_adj)) +
  ...
  annotate("text", x = c(34), 
           y = c(15), 
           adj = 0, label = paste0("5%-95%\nrange = ", round(quantile(res.ndvi$ed50.ndvi_adj,probs=c(.05,.5, .95))[3]-quantile(res.ndvi$ed50.ndvi_adj,probs=c(.05,.5, .95))[1], 2)), size = 5)
...
ggsave(all_plots,filename="Figures/Supplementary_Fig4.png",width=12,height=6,dpi=320)
```
