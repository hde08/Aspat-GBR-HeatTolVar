...
f0_ndvi=df_hyper %>% subset(MeanNDVI<0.6) %>% ggplot(aes(x=mF,y=MeanNDVI)) + geom_point(size=3,alpha=0.6)+ theme_linedraw()+ labs(x="F0",y="NDVI")+ theme(axis.text.x=element_text(size=20,color="black"),axis.text.y=element_text(size=20,color="black"),axis.title.x = element_text(size=18,color="black"),axis.title.y = element_text(size=20,color="black"),legend.title = element_text(size=15,color="black"),legend.text = element_text(size=20,color="black"),legend.position = "none",legend.background = element_rect(fill = "transparent"),
...
ggsave(f0_ndvi,filename="C:/Users/Hugo/Documents/Figures/GBR_ED50/F0_NDVI_inf0.6.png",width=12,height=10,dpi=320)
...
df_hyper$Site.name=factor(df_hyper$Site.name,levels=sites_data$Site.name[order(sites_data$MMM)])
  
f0_hist=df_hyper %>% merge(sites_data %>% dplyr::select(Site.name,MMM),by="Site.name") %>% ggplot(aes(x=mF)) + geom_histogram(aes(fill = MMM,group=MMM))+ theme_linedraw()+ labs(x="F0",y="N")+ theme(axis.text.x=element_text(size=20,color="black"),axis.text.y=element_text(size=20,color="black"),axis.title.x = element_text(size=18,color="black"),axis.title.y = element_text(size=20,color="black"),legend.title = element_text(size=15,color="black"),legend.text = element_text(size=20,color="black"),legend.position = "right",legend.background = element_rect(fill = "transparent"),
          panel.background = element_rect(colour = "black", size=1.5))+ facet_wrap(.~Treatment) + 
       theme(strip.text.x = element_text(size = 25)) +scale_fill_gradientn(colors=new_palette) 

ggsave(f0_hist,filename="C:/Users/Hugo/Documents/Figures/GBR_ED50/F0_distrib_wcolor.png",width=12,height=10,dpi=320)

ndvi_hist=df_hyper %>% merge(sites_data %>% dplyr::select(Site.name,MMM),by="Site.name") %>% ggplot(aes(x=MeanNDVI)) + geom_histogram(aes(fill = MMM,group=MMM))+ theme_linedraw()+ labs(x="NDVI",y="N")+ theme(axis.text.x=element_text(size=20,color="black"),axis.text.y=element_text(size=20,color="black"),axis.title.x = element_text(size=18,color="black"),axis.title.y = element_text(size=20,color="black"),legend.title = element_text(size=15,color="black"),legend.text = element_text(size=20,color="black"),legend.position = "none",legend.background = element_rect(fill = "transparent"),
          panel.background = element_rect(colour = "black", size=1.5))+ facet_wrap(.~Treatment) + 
       theme(strip.text.x = element_text(size = 25)) +scale_fill_gradientn(colors=new_palette) 

ggsave(ndvi_hist,filename="C:/Users/Hugo/Documents/Figures/GBR_ED50/NDVI_distrib_wcolor.png",width=12,height=10,dpi=320)
```
